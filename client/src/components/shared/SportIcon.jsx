...
export const RunnerSvg = ({ className = '', style }) => (
  <svg viewBox="0 0 36 38" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
...
/**
 * Low-level sport glyph — same shapes everywhere (lists, tiles, pills).
 * Pass `color` to override the default sport tint (e.g. white on selected pill).
 */
export function SportGlyph({ sport, size, color, className = '' }) {
  if (!sport && sport !== 0) return null;
  const key = resolveSportKey(sport);
  const tint = color || SPORT_ICON_COLORS[key] || SPORT_ICON_COLORS.other;
  const cls = `${className} flex-shrink-0`.trim();
  const stroke = 2;
...
/**
 * Shared sport icon component — matches CalendarView styling.
 * sport: any string (e.g. 'bike', 'run', 'swim', 'Hike', 'strength', etc.)
 * className: size + additional Tailwind classes (default "w-4 h-4")
 */
const SportIcon = ({ sport, className = 'w-4 h-4', color }) => {
  if (!sport) return null;
  const key = resolveSportKey(sport);
  const tw = color ? '' : (SPORT_TAILWIND[key] || SPORT_TAILWIND.other);
  return (
    <SportGlyph
      sport={sport}
      color={color}
      className={`${className} ${tw}`.trim()}
    />
  );
};
