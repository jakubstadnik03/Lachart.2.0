 ...
 *
 * CommonJS twin of client/src/utils/hrPowerProfile.js, produced by
 ...
 */
function thresholdToDemand(value, { kind, storageMode }) {
...
function demandToThreshold(demand, { kind, storageMode }) {
 ...
 */
function gradeFactor(gradeFraction) {
 ...
 */
function toSeries(records, sport) {
 ...
 */
function steadySegments(series, { lt2Demand, lt1Demand, maxDemandCv }) {
 ...
 */
function sessionCloud(series, { binSec = 30 } = {}) {
  if (!series) return [];
  const { demand, hr, n } = series;
  const out = [];
 ...
 */
function testLactateCurve(anchor) {
 ...
 */
function lactateCurveShift(anchor, samples = []) {
 ...
 *                   verdict:string}}
 */
function zoneAgreement(cloud, { demandBounds, hrBounds } = {}) {
 ...
 */
function sessionIntent({ title = '', plannedTarget = null } = {}) {
 ...
 */
function judgeThresholdSplit(intent) {
  const neutral = {
 ...
 */
function fitSession(points, { slopePrior, lt2Demand }) {
 ...
 */
function analyseSession({ records, sport, anchor, tempC = null, slopeFit = null }) {
 ...
 */
function localSlopeAt(curve, demand) {
  const pts = curve?.points;
 ...
 */
function shiftedLactateCurve(anchor, projection) {
  const curve = testLactateCurve(anchor);
 ...
 */
function zoneAdviceFor(projection, { testDate = null, now = null } = {}) {
 ...
 */
function buildDriftHistory(sessions, { testDate, windowDays = 28 } = {}) {
